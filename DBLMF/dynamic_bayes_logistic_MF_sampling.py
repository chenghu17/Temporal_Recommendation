...
class generate_data():
    def __init__(self, datatype, interval, userNum, itemNum):
        ...
        self.timestamp = interval * 30 * 24 * 3600
...
class DBLMF():
    def __init__(self, *args):
        ...
        fh = logging.FileHandler('sDBLMF_' + self.datatype + '_itera_' + str(self.itera), mode='a', encoding=None,
                                 delay=False)
    ...
    def uniform_sampling(self):
        self.train_negtive_user_key = [list() for _ in range(self.userNum)]  # 每个用户，在每个时间段，打分item_id集合，
        self.train_negtive_item_key = [list() for _ in range(self.itemNum)]  # 每个item，在每个时间段，被哪些user打过分

        for userid in range(self.userNum):
            self.train_negtive_user_key[userid] = [set() for _ in range(self.interval_num)]
        for itemid in range(self.itemNum):
            self.train_negtive_item_key[itemid] = [set() for _ in range(self.interval_num)]

        for t in range(self.interval_num):
            # Si表示当前时间user i 没有打过分的item
            current_user_set = self.train_time_userSet[t]
            for uid in current_user_set:
                current_user_item_set = set(self.train_user_key[uid][t])
                negative_item_set = self.train_itemSet - current_user_item_set
                if len(current_user_item_set) > len(negative_item_set):
                    self.train_negtive_user_key[uid][t] = set(negative_item_set)
                else:
                    self.train_negtive_user_key[uid][t] = set(
                        random.sample(negative_item_set, len(current_user_item_set)))
            # Tj表示当前时间没有对item j打过分的user
            current_item_set = self.train_time_itemSet[t]
            for item_id in current_item_set:
                current_item_user_set = set(self.train_item_key[item_id][t])
                negative_user_set = self.train_userSet - current_item_user_set
                if len(current_item_user_set) > len(negative_user_set):
                    self.train_negtive_item_key[item_id][t] = set(negative_user_set)
                else:
                    self.train_negtive_item_key[item_id][t] = set(
                        random.sample(negative_user_set, len(current_item_user_set)))
            ...
            for itera in range(self.itera):
                ...
                for uid in range(self.userNum):
                    ...
                    if uid in current_user_set:
                        ...
                        negative_current_user_item_set = set(self.train_negtive_user_key[uid][t])
                        # compute auxiliary variables EPSILON(i,j)
                        for item_id in positive_current_user_item_set | negative_current_user_item_set:
                        ...
                        for k in range(self.dimension):
                            ...
                            for item_id in list(positive_current_user_item_set):
                                part1 += self.itemMat[t][item_id][k] * (
                                        1 + self.delta * 1 + 1 - 4 * (1 + self.delta * 1 - 1) * self.self_lambda(
                                    self.EPSILON[t][uid][item_id]) * self.Tijk(self.userMat[t][uid],
                                                                               self.itemMat[t][item_id], k))
                            # part2
                            part2 = 0
                            ...
                            # update
                            self.userMat[t][uid][k] = (1 - self.gamma) * self.userMat[t][uid][k] + self.gamma * (
                                    0.5 * part1 - (self.itemNum / (2 * len(negative_current_user_item_set))) * part2
                                    + part3) * self.SIGMA[t][uid][k]
                    else:
                        if t != 0:
                            for k in range(self.dimension):
                                self.userMat[t][uid][k] = (1 - self.gamma) * self.userMat[t][uid][k] + self.gamma * (
                                        self.userMat[t - 1][uid][k] * self.alpha[k]) * self.SIGMA[t][uid][k]
                        else:
                            self.userMat[t][uid] = self.userMat[t][uid]
                    ...
                    if uid in current_user_set:
                        ...
                        for k in range(self.dimension):
                            ...
                            part1 = 0
                            for item_id in list(positive_current_user_item_set):
                                part1 += (1 + self.delta * 1 - 1) * self.self_lambda(self.EPSILON[t][uid][item_id]) * (
                            ...
                            for item_id in list(negative_current_user_item_set):
                                part2 += self.self_lambda(self.EPSILON[t][uid][item_id]) * (
                                        np.square(self.itemMat[t][item_id][k]) + self.OMEGA[t][item_id][k])
                            # update
                            self.SIGMA[t][uid][k] = 1 / ((1 - self.gamma) * self.SIGMA[t][uid][k] + self.gamma * (
                                    self.alpha[k] + self.sigma0 + 2 * part1 + 2 * self.itemNum / len(
                                negative_current_user_item_set) * part2))
                    else:
                        self.SIGMA[t][uid] = self.SIGMA[t][uid]

                # ----------------------------------------------------------------------------------------

                # 需要使用对该item打过分的user集合
                ...
                for item_id in range(self.itemNum):
                    ...
                    if item_id in current_item_set:
                        ...
                        negative_current_item_user_set = set(self.train_negtive_item_key[item_id][t])
                        for uid in positive_current_item_user_set | negative_current_item_user_set:
                            tmp1 = self.SIGMA[t][uid] * self.SIGMA[t][uid] + self.userMat[t][uid] * self.userMat[t][uid]
                            tmp2 = self.OMEGA[t][item_id] * self.OMEGA[t][item_id] + self.itemMat[t][item_id] * \
                                   self.itemMat[t][item_id]
                            part1 = np.matmul(tmp1, tmp2)
                        ...
                        for k in range(self.dimension):
                            ...
                            for uid in list(positive_current_item_user_set):
                                part1 += self.userMat[t][uid][k] * (
                                        1 + self.delta * 1 + 1 - 4 * (1 + self.delta * 1 - 1) * self.self_lambda(
                                    self.EPSILON[t][uid][item_id]) * self.Tijk(
                                    self.userMat[t][uid], self.itemMat[t][item_id], k))
                            ...
                            # update
                            self.itemMat[t][item_id][k] = (1 - self.gamma) * self.itemMat[t][item_id][k] \
                                                          + self.gamma * (0.5 * part1 - (
                                    self.userNum / (2 * len(negative_current_item_user_set))) * part2 + part3) * \
                                                          self.OMEGA[t][item_id][k]
                    else:
                        if t != 0:
                            for k in range(self.dimension):
                                self.itemMat[t][item_id][k] = (1 - self.gamma) * self.itemMat[t][item_id][
                                    k] + self.gamma * self.itemMat[t - 1][item_id][k] * self.alpha[k] * \
                                                              self.OMEGA[t][item_id][k]
                        else:
                            self.itemMat[t][item_id] = self.itemMat[t][item_id]
                        ...
                        for k in range(self.dimension):
                            ...
                            for uid in list(negative_current_item_user_set):
                                part2 += self.self_lambda(self.EPSILON[t][uid][item_id]) * (
                                        np.square(self.userMat[t][uid][k]) + self.SIGMA[t][uid][k])
                            # update
                            self.OMEGA[t][item_id][k] = 1 / (
                                    (1 - self.gamma) * self.OMEGA[t][item_id][k] + self.gamma * (
                                    self.beta[k] + self.sigma0 + 2 * part1 + 2 * self.userNum / len(
                                negative_current_item_user_set) * part2))

                    else:
                        self.OMEGA[t][item_id] = self.OMEGA[t][item_id]
...
if __name__ == '__main__':
    ...
    gamma = 0.95  # for update

    model = DBLMF(datatype, month, global_dim, itera, user, item, sigma0, a0, a1, b0, b1, delta, gamma)
    model.VI_for_DBLMF()
